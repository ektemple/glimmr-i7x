...
[The hack here inserts a hook to provide a message when the player abandons an open question.]
...
Section - New Yes/No question routine (in place of Section 5 - Processing yes/no questions in Questions by Michael Callaghan)
...
A thing can be wide-open or secreted. A thing is usually wide-open.

The stroke highlight color is in Fake-room_x. It is secreted.
The canvas background color is in Fake-room_x. It is secreted. Understand "canvas color" as the canvas background color.
The outline color is in Fake-room_x. It is secreted.
...
Carry out changing the color of:
	if the noun is:
		-- highlight color: change the highlight-color to the glulx color value understood;
...
Carry out listing available colors:
	say "The colors of some elements of the interface can be changed. These include [list of secreted things]. The canvas background color will appear in the source code output; all of the others are purely UI elements. Any of these colors can also be used to change the foreground color and background color for drawing. The available colors are:[paragraph break]";
...
A number question rule when the current action is selecting highlight type (this is the highlight selection rule):
...
Interrogation is a kind of value. The interrogations are q-window, q-width, q-height, and q-complete.
...
The startup text is text that varies. Startup text is "Welcome to the Glimmr Canvas Editor. This is a utility intended to help you arrange graphical elements into a composition for use with the Glimmr library and the Flexible Windows extension. Once your composition is complete, you may export complete, compilable source code that will replicate your composition using Inform.[paragraph break]"
...
This is the print the editor startup text rule:
	...
	now stage is q-window;
	follow the intro quiz control rule.

The startup instructions text is text that varies. The startup instructions text is "To begin, maximize your window to its full size. Then, answer the following questions. (If you prefer, you can just type SKIP to go straight to the editor; all of these settings can be also be specified directly from the editor.)[paragraph break]".
...
Every turn when quiz-ongoing is true (this is the intro quiz control rule):
	if stage is q-window:
		now current question is "[italic type]What is the name of the Flexible Windows g-window these sprites will be associated with? (This will customize the output source code so that your sprites appear in the correct Flexible Windows graphics window.)[roman type]";
		now current prompt is "Enter the name of a g-window (e.g., graphics-window): >";
		ask a closed question, in text mode;
	if stage is q-width:
		now current question is "[italic type]What is the width (in pixels) of your background grid? If you intend to use one of the images in the editor's library to define your background, you may type SKIP to go directly to the editor.[roman type]";
		now current prompt is "Enter a number: >";
		ask a closed question, in number mode;
	if stage is q-height:
		now current question is "[italic type]What is the height (in pixels) of your background grid?[roman type]";
		now current prompt is "Enter a number: >";
		ask a closed question, in number mode;
...
A text question rule when the stage is q-window (this is the window-prompting rule):
	now targeted window is the current answer;
	if targeted window is "":
		change targeted window to "graphics-window";
	say "Thank you. Sprites will be assigned to the [targeted window]. The output source code will set up a window of this name, but you will likely want to customize the provided code.";
...
A number question rule when the stage is q-width (this is the width-prompting rule):
	if the number understood is less than 1:
		...
		retry;
	say "Thank you. The grid will be [the number understood] pixels wide.";
...
A number question rule when the stage is q-height (this is the height-prompting rule):
	if the number understood is less than 1:
		say "Please enter a number greater than 0.[line break]";
		retry;
	say "Thank you. The grid will be [the number understood] pixels high.";
...
graphics-window	g-placeabove	60	g-placenullcol	graphics-canvas
...
The main-window spawns the graphics-window. The graphics-window spawns the layers-window.
The graphics-window spawns the drawing-window.
...
A window-drawing rule for the graphics-window (this is the graphics-window drawing rule):
	if the graphics-window is g-present:
		...
		if canvas-outlining is true:
			draw a box (color Canvas outline-color) in the graphics-window from (x-offset of the graphics-window) by (y-offset of the graphics-window) to (the x-offset of the graphics-window plus the scaled width of the graphics-canvas) by (the y-offset of the graphics-window plus the scaled height of the graphics-canvas) with 1 pixel line-weight, inset;
		carry out the drawing the active elements activity with the graphics-window;
...
This is the editor startup rule:
	...
	set the background color of the status window to g-darkpear;
...
To set windows to portrait orientation:
	...
	now the measurement of the graphics-window is 55;
...
To decide which text is the coordinates of the selected component:
	if the element-selection set is empty:
		decide on "";
	if the number of entries of the element-selection set is 1:
...
A dynamic-rectangle is a kind of rectangle primitive. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-primitive is rectangle.
...
A dynamic-box is a kind of box primitive. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-primitive is box.
...
A dynamic-stroked-rectangle is a kind of stroked rectangle primitive. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-primitive is stroked rectangle.
...
A dynamic-line is a kind of line primitive. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-primitive is line.
...
A dynamic-bitmap-string is a kind of bitmap-rendered string. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-string is bitmap string.
...
A dynamic-image-string is a kind of image-rendered string. The display status is g-inactive. The associated canvas is the graphics-canvas. The g-string is image string.
...
To page and scale the/-- library-window sprites: 
	if the library-window is g-present:
		change canvas-width of the library-canvas to the width of the library-window;
		change canvas-height of the library-canvas to the height of the library-window;
	...
	while (suspend_flag) {
		...
		if (ix >= 0 && gg_event-->0 == 2) { 
			...
			if (key == $fffffff8) {
				...
				!print "[Element drawing canceled]"; new_line;
...
A number question rule when the current action is interactively changing the line-weight:
	if the number understood is less than 1:
		say "The line-weight must be greater than 0.";
		retry;
...
Deselecting all is an action applying to nothing. Understand "deselect all" or "deselect" or "none" or "d" as deselecting all. [Deselects all visible sprites in the graphics-window.]
...
A number question rule when the current action is moving interactively:
...
A number question rule when the current action is interactively changing the nudge-factor:
...
A text question rule when the current action is interactively changing the name of:
...
A text question rule when the current action is interactively changing the kind of:
...
A text question rule when the current action is interactively tagging:
...
A text question rule when the current action is interactive graphlinking:
	let L be a list of indexed text;
	let L be {};
	repeat with current-element running through the element-selection set:
...
A real number question rule when the current action is interactively setting the scaling factor:
	abide by the scaling limit validation rule;
...
A real number question rule when the current action is interactive symmetrical scaling:
...
A real number question rule when the current action is interactive asymmetrical scaling:
	abide by the scaling limit validation rule;
...
Carry out changing the current display-layer:
	...
	follow the window-drawing rules for the control-window;
	[follow the window-drawing rules for the layers-window];
	say "New sprites will be created on display-layer [current display-layer].";
...
A number question rule when the current action is interactively changing the display-layer:
	if the number understood is less than 1 or the number understood is greater than the maximum display-layer:
		say "Please enter a number between 1 and [maximum display-layer].";
		retry;
...
Carry out revealing layers:
	...
	[follow the window-drawing rules for the layers-window;]
	follow the window-drawing rules for the working window;
	say "Layer-reveal mode is now active. The display-layer of each element in the editing window is identified by a number in the upper-left corner of the element.";
...
A text question rule when the current action is indicating an element instance:
	abide by the element instance registration rule;
	exit.
...
A number question rule when the current action is deleting an element instance:
...
A number question rule when the current action is loading an element instance:
	abide by the instance menu input validation rule;
	now the current instance is the instance-name corresponding to an instance-number of the number understood in the Table of Current Element Instances;
...
Carry out centering elements on:
	say "Please click on a element in the editing window to center the selected element on that element. Cancel by clicking on any other button.";
...
Carry out aligning elements:
	say "Please click on an element in the editing window to align the selected element to the [border-name understood] of that element. Cancel by clicking on any other button.";
...
Renaming the targeted window is an action applying to one topic. Understand "rename the target window [text]" or "rename window [text]" or "window [text]" or "rename target window [text]" as renaming the targeted window.
		
Carry out renaming the targeted window:
	let T be indexed text;
	let T be the topic understood;
	replace the text "[quotation mark]" in T with "";
	change the targeted window to T;
	say "Source code will be generated targeting the g-window [italic type][targeted window][roman type]".
...
To write the/-- preamble:
	append "Include Glimmr Canvas-Based Drawing by Erik Temple.[line break]Include Glimmr Drawing Commands by Erik Temple." to the file of Output;
	append "[if we have graphic links][line break]Include Glimmr Graphic Hyperlinks  by Erik Temple.[end if][if we have primitives][line break]Include Glimmr Element Primitives by Erik Temple.[end if][if we have rendered strings][line break]Include Glimmr Text-Painting Elements by Erik Temple.[font extensions][end if][paragraph break]" to the file of Output;
	...
	if we have customized colors:
		...
		repeat with current-hue running through the drawing colors:
			...
			if the back-colour of the working window is not a glulx color value listed in the Table of Default Color Values and the back-colour of the working window is not listed in the drawing colors:
			 	append "[back-colour of working window][tab][assigned number of back-colour of working window][line break]" to the file of Output;
...
To write paragraphed source code:
	repeat with current-list running through the kind-parsed omnibus:
		...
		append "The associated canvas of [kind-article] [current-kind] is [targeted window] canvas. " to the file of Output;
		append "[paragraph break]" to the file of Output;
...
Carry out previewing source code:
	say "[italic type]Inform cannot print tabs to the window, so previewed code will use spaces instead. Type GENERATE SOURCE CODE to write a file with tabs to your hard drive. Here is the source code:[roman type][line break]";
	follow the source code generation rule;
...
Example: *** Basic Floorplan - A toolset including images for use in constructing basic floorplans for indoor maps, as well as illustrating most of the basic options available. Also illustrates the use of user-specified source text and tags to create automatic mapping capabilities.
	...
	*: "Basic Floorplan"
	...
	The targeted window is "graphics-window".

	The canvas-width of the graphics-canvas is 500.
	The canvas-height of the graphics-canvas is 300.
	...
	The user-specified source text is "Chapter - Associating rooms with elements	A g-element has a room called the associated room.	The update map sprites rule is listed in the carry out looking rules.	This is the update map sprites rule:	[tab]repeat with current-element running through display-inactive g-elements:	[tab][tab]if the associated room of current-element is the location:	[tab][tab][tab]activate current-element;	[tab]follow the window-drawing rules for the editor-window."


	Section - Map

	Square Room is a room. It is west of Long Room.

	Circle Room is west of Square Room. Square Room is north of Circle Room. South of Square Room is nowhere. East of Circle Room is nowhere.

	Hexagonal Room is south of Long Room. The red door is an open door. Red door is south of Hexagonal Room and north of Boxy Room.
